...
internal class UmbrellaDynamicImageConverter : UmbrellaResponsiveImageConverter
{
		...
		: base(maxPixelDensity, innerConverter)
	{
		...
		DynamicImageUtility = UmbrellaMauiServices.GetService<IDynamicImageUtility>();
	}
	...
	public override object? Convert(object? value, Type targetType, object? parameter, CultureInfo culture)
	{
		if (value is string imageUrl)
		{
			...
			string path = DynamicImageUtility.GenerateVirtualPath(DynamicImagePathPrefix, options).TrimStart('~').Replace("//", "/", StringComparison.Ordinal);
