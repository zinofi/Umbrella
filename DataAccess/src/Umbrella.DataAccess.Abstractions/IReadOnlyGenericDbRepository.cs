...
namespace Umbrella.DataAccess.Abstractions
{
	...
	public interface IReadOnlyGenericDbRepository<TEntity, in TRepoOptions, TEntityKey> : IReadOnlyGenericDbRepository<TEntity, TRepoOptions, TEntityKey, TEntity>
	...
	public interface IReadOnlyGenericDbRepository<TEntity, in TRepoOptions, TEntityKey, TSlimEntity>
		where TEntity : class, IEntity<TEntityKey>
		where TRepoOptions : RepoOptions, new()
		where TEntityKey : IEquatable<TEntityKey>
		where TSlimEntity : class, IEntity<TEntityKey>
	{
		...
		/// <param name="additionalFilterExpressions">Optional additional filter expressions that are too complex to model using the <see cref="FilterExpression{TItem}"/> type.</param>
		/// <returns>The paginated results.</returns>
		Task<PaginatedResultModel<TSlimEntity>> FindAllSlimAsync(
			int pageNumber = 0,
			int pageSize = 20,
			CancellationToken cancellationToken = default,
		...
		/// <summary>
		/// Finds a slim entity in the database using its id.
		/// </summary>
		/// <param name="id">The identifier.</param>
		/// <param name="cancellationToken">The cancellation token.</param>
		/// <returns>The entity.</returns>
		Task<TSlimEntity?> FindSlimByIdAsync(TEntityKey id, CancellationToken cancellationToken = default);
