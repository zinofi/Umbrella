...
namespace Umbrella.Legacy.WebUtilities.DynamicImage.Middleware
{
	...
	{
		private static readonly ILog s_Log = LogManager.GetLogger(typeof(DynamicImageMiddleware));

		public DynamicImageMiddleware(OwinMiddleware next)
			: base(next)
		{
			if (s_Log.IsDebugEnabled)
				s_Log.Debug("DynamicImageMiddleware registered successfully");
		}
		...
		public override async Task Invoke(IOwinContext context)
		{
			...
			catch (Exception exc)
			{
				...
				//404 needs to be sent outside the catch block due to limitations on using the await
				//keyword in catch blocks
				s_Log.Error("Error in DynamicImageModule for path: " + context.Request.Path, exc);
