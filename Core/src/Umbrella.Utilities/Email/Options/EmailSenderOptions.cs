...
namespace Umbrella.Utilities.Email.Options
{
	...
	public class EmailSenderOptions : ISanitizableUmbrellaOptions, IValidatableUmbrellaOptions
	{
		...
		{
			Guard.ArgumentNotNullOrWhiteSpace(DefaultFromAddress, nameof(DefaultFromAddress));

			switch (DeliveryMethod)
			{
				case EmailSenderDeliveryMode.Network:
					Guard.ArgumentNotNullOrWhiteSpace(Host, nameof(Host));

					if (!string.IsNullOrWhiteSpace(UserName))
						Guard.ArgumentNotNullOrWhiteSpace(Password, nameof(Password));
					break;
				case EmailSenderDeliveryMode.SpecifiedPickupDirectory:
					Guard.ArgumentNotNullOrWhiteSpace(PickupDirectoryLocation, nameof(PickupDirectoryLocation));
					break;
			}
		}
	}
