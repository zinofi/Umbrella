...
public class MimeTypeUtility : IMimeTypeUtility
{
	...
	private static readonly Dictionary<string, string> _extensionToMimeTypeDictionary = new()
	{
	...
	private static readonly Dictionary<string, string> _mimeTypeToExtensionDictionary = ReverseDictionary(_extensionToMimeTypeDictionary);
	...
	public string GetMimeType(string fileNameOrExtension)
	{
		...
		try
		{
			...
			return _extensionToMimeTypeDictionary.TryGetValue(destinationSpan.ToString(), out string? mimeType) ? mimeType : DefaultMimeType;
		}
	...
	public string? GetFileExtension(string? mimeType)
	{
		try
		{
			return !string.IsNullOrWhiteSpace(mimeType) && _mimeTypeToExtensionDictionary.TryGetValue(mimeType!, out string? extension) ? extension : null;
		}
	...
	private static Dictionary<string, string> ReverseDictionary(Dictionary<string, string> dictionary)
	{
		...
		{
			if (!reversedDictionary.TryGetValue(kvp.Value, out string? existingKey))
			{
				reversedDictionary.Add(kvp.Value, kvp.Key);
			}
			else
			{
				if (existingKey.Length > kvp.Key.Length)
				{
