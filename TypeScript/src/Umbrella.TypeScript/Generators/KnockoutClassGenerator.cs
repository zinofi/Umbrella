					...
					case RequiredIfEmptyAttribute attr:
						{
							...
							validationBuilder.AppendLineWithTabIndent($"required: {{ onlyIf: () => this.{dependentPropertyName} === undefined || this.{dependentPropertyName} === null || (typeof property === \"number\" && isNaN(this.{dependentPropertyName}!)) || (typeof property === \"string\" && this.{dependentPropertyName}!.trim().length === 0), message: {message} }},", indent);
						}
						break;
